...
describe('UserService', () => {
  ...
  const payload: IcreateUserViewDto = { email: 'test@test.com', password: 'Password1', name: 'teste' };
  const createdUser: UserDto = { email: payload.email,name: payload.name, password: payload.password,role: 'USER', id: '1', accountId: '1', createdAt: new Date('2024-10-16T15:58:26.135Z')};
  const createdAccount: AccountDto = { id: 'account-id', balance: new Decimal(100),lockedBalance: new Decimal(0),createdAt:  new Date('2024-10-16T15:58:26.135Z') };
  ...
  describe('create', () => {
    it('should throw an error if the password is invalid', async () => {
      const payload: IcreateUserViewDto = { email: 'test@test.com', password: '12345' , name: 'teste'};
      await expect(userService.create(payload)).rejects.toThrow(BadRequestException);
    });
    ...
    it('should create a user and account successfully', async () => {
      ...
      jest.spyOn(userRepository, 'update').mockResolvedValue({ ...createdUser, accountId: createdAccount.id });
  ...
  describe('initiateTransfer', () => {
    it('should throw an error if sender is the same as receiver', async () => {
      await expect(userService.initiateTransfer({ sender: '1', receiver: '1', amount: 100 })).rejects.toThrow(TypeError);
    });
    ...
    it('should throw an error if sender has insufficient funds', async () => {
      ...
      jest.spyOn(userRepository, 'showBalance').mockResolvedValue(createdAccount);
      jest.spyOn(userRepository, 'findByEmail').mockResolvedValue(createdUser);

      await expect(userService.initiateTransfer({ sender: '1', receiver: '2', amount: 100 })).rejects.toThrow(BadRequestException);
    });
    ...
    it('should initiate a transfer successfully', async () => {
      ...
      jest.spyOn(userRepository, 'showBalance').mockResolvedValue(createdAccount);
      jest.spyOn(userRepository, 'findByEmail').mockResolvedValue(createdUser);
      jest.spyOn(transactionService, 'initiateTransfer').mockResolvedValue(createTransactionDto);

      const result = await userService.initiateTransfer({ sender: '3', receiver: '2', amount: 5 });
...
;
    ...
    it('should confirm the transfer successfully', async () => {
      ...
      const result = await userService.confirmTransfer(createTransactionDto, 'user-id');
  ...
  describe('reversalTransfer', () => {
    it('should throw an error if reversal transaction ID already exists', async () => {
      jest.spyOn(transactionService, 'findTransactionReversal').mockResolvedValue(createTransactionDto);

      await expect(userService.reversalTransfer(createTransactionDto)).rejects.toThrow(BadRequestException);
    });
    ...
    it('should process the reversal successfully', async () => {
      const completedTransaction = { id: '1', status: TransactionStatus.COMPLETE, value: new Decimal(500), debitedAccountId: 'debited-id', creditedAccountId: 'credited-id',transactionType: TransactionType.TRANSFER, createdAt:new Date('2024-10-16T15:58:26.135Z') };

      jest.spyOn(transactionService, 'findTransaction').mockResolvedValue(completedTransaction);
      jest.spyOn(transactionService, 'findTransactionReversal').mockResolvedValue(null);
      jest.spyOn(accountService, 'updateAccountBalance').mockResolvedValue(undefined);
      jest.spyOn(transactionService, 'initiateTransfer').mockResolvedValue(createTransactionDto);
